...
- (instancetype) initWithDelegate:(id<AUIRecorderRateSelectViewDelegate>)delegate {
    ...
    if (self) {
        ...
        _selectedView.backgroundColor = AUIFoundationColor(@"fill_ultraweak");
...
- (void) setupStackView {
    ...
    for (int i = 0; i < RateCount; ++i) {
        ...
        [btn setTitleColor:AUIFoundationColor(@"text_infrared") forState:UIControlStateSelected];
...
- (void) onBtnDidPressed:(UIButton *)btn {
    ...
    if ([_delegate respondsToSelector:@selector(onAUIRecorderRateSelectView:rate:)]) {
        [_delegate onAUIRecorderRateSelectView:self rate:self.rate];
    }
}
