...
- (void)loadCachePreload {
    [AUIVideoCachePreloadTool setPreloadConfig:self];
    [AUIVideoCachePreloadTool preloadUrl:self.sources.firstObject.url];
}
...
- (void)startPreloadNextPlayAtPlayer:(AliPlayer *)player {
    ...
    if (self.currentPlayIndex + 1 < self.sources.count) {
        [AUIVideoCachePreloadTool cancelPreloadUrl:self.sources[self.currentPlayIndex].url];
        if (![self.sources[self.currentPlayIndex + 1].url isEqualToString:self.sources[self.currentPlayIndex].url]) {
            [AUIVideoCachePreloadTool cancelPreloadUrl:self.sources[self.currentPlayIndex + 1].url];
            [AUIVideoCachePreloadTool preloadUrl:self.sources[self.currentPlayIndex + 1].url];
        }
    }
}
...
- (void)onCompleted:(NSString *)url {
    dispatch_async(dispatch_get_main_queue(), ^{
        [AUIVideoCachePreloadTool cancelPreloadUrl:url];
    ...
    if (curModel) {
        ...
        if (index + 1 < self.sources.count) {
            if (![self.sources[index + 1].url isEqualToString:url]) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [AUIVideoCachePreloadTool cancelPreloadUrl:self.sources[index + 1].url];
                    ...
                    if (currCell.player.bufferedPosition > 5 * 1000) {
                        [AUIVideoCachePreloadTool preloadUrl:self.sources[index + 1].url];
                    }
                });
            }
        }
...
- (void)destroyPlay {
    ...
    [AUIVideoCachePreloadTool cancelPreloadUrl:nil];
    for (AliPlayer *player in self.playerSaver) {
        [player stop];
        [player destroy];
    }
